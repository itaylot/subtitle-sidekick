...
| קובץ | מה | מתי להשתמש |
|------|-----|-----------|
| `transcribe_to_srt.py` | ⭐ מתמלל קובץ הרצאה → **SRT** מסונכרן | **הדרך המומלצת** להרצאות מוקלטות (Moodle) |
| `server.py` | שרת תמלול **חי** (faster-whisper + VAD) | לתוסף הכתוביות החיות (מומלץ עם GPU) |
| `mock_server.py` | מחזיר טקסט עברי **מזויף**, בלי ML | לבדוק את התוסף מקצה-לקצה בלי הורדת מודל |

### תמלול הרצאה לקובץ כתוביות (מומלץ)
```bash
python transcribe_to_srt.py "הרצאה.mp4"     # יוצר הרצאה.srt
```
תומך בכל פורמט (mp4/mkv/mp3/m4a/wav). פותחים את הווידאו ב-VLC או ב-`tools/player.html`.
